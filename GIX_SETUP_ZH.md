...
自动任务每天北京时间约 05:17 首次启动，06:17 再做一次兜底检查；如果当天中文日报已经生成，第二次任务会自动跳过，不会重复向飞书推送。日报标题和归档日期使用北京时间。
...
- 如果没有内容达到 Profile 阈值，系统会明确标记并保底展示有效评分最高的 2 条；如果 AI 对全部内容均未产生有效评分，系统仍会推送按时间与来源多样性选择的 2 条原始候选，并明确提示检查 DeepSeek API 密钥与账户余额，不再静默生成空报。
